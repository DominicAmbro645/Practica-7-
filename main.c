...
void dfs_topol_traverse(Graph* g, Vertex* v, List* salida_topol, size_t* time)
{
    *time += 1;
    ...
    if (Vertex_HasNeighbors(v))
    {
        for (Vertex_Start(v); !Vertex_End(v); Vertex_Next(v))
        {
            ...
            if (w->color == WHITE)
            {
                w->predecessor = v->data;
                dfs_topol_traverse(g, w, salida_topol, time);
            }
        }
    }
    ...
    *time += 1;
    ...
    // Agregar el vértice a la lista de salida topológica cuando se termine de visitar
    List_Push_back(salida_topol, v->data, 0.0);
}

void dfs_topol(Graph* g, int start)
{
    size_t time = 0;
    List* salida_topol = List_New(); // Lista para el orden topológico
    ...
    Vertex* start_vertex = Graph_GetVertexByKey(g, start);
    if (start_vertex)
    {
        dfs_topol_traverse(g, start_vertex, salida_topol, &time);
    }
    
    for (int i = 0; i < g->len; ++i)
    {
        Vertex* v = Graph_GetVertexByIndex(g, i);
        if (Vertex_GetColor(v) == WHITE) // Si el vértice no fue visitado
        {
            dfs_topol_traverse(g, v, salida_topol, &time); // Procesar este vértice
        }
    }

    printf("Orden Topológico: ");
    List_Cursor_front(salida_topol);

    printf("\n");
    List_Delete(&salida_topol);
}

void Graph_Print_dfs(Graph* g)
{
    printf("Impresión del grafo (tiempos de descubrimiento y finalización):\n");
    for (int i = 0; i < g->len; ++i)
    {
        Vertex* vertex = &g->vertices[i];
        // Imprimir vértice y sus vecinos
        printf("[%d] %d => ", i, vertex->data);
        if (vertex->neighbors)
        {
            for (List_Cursor_front(vertex->neighbors);
        ...
        printf("    Descubrimiento: %d, Finalización: %d\n",
               vertex->discovery_time, vertex->finish_time);
    }
    printf("\n");
}
